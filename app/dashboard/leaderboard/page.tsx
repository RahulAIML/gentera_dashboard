...
export default function LeaderboardPage() {
  ...
  return (
    <div className="min-h-full bg-surface-950">
      <TopBar title={t.nav.leaderboard} subtitle={t.scope.organization} />

      <div className="p-6 lg:p-8 space-y-6 max-w-[1400px] mx-auto">
        {/* KPIs */}
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          <KPICard title={t.charts.leaderboard} value={fmtNumber(userLeaderboard.length)} icon={Users} accent="brand" index={0} loading={isLoading} />
          ...
          <KPICard title={t.kpi.totalActivities} value={fmtNumber(activityLeaderboard.length)} icon={BarChart3} accent="violet" index={2} loading={isLoading} />
        ...
        {!isLoading && userLeaderboard.length >= 3 && (
          <div className="rounded-2xl border border-border bg-surface-900/60 p-6">
          ...
          {(["users", "activities"] as const).map((tb) => (
            <button
              ...
              className={cn(
                ...
                tab === tb ? "bg-brand-500/10 border-brand-500/40 text-brand-400" : "border-border text-text-muted hover:border-border-strong"
              )}
            >
