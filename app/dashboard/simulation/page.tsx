...
import { computeKPISummary, computeMonthlyTrendLocalized } from "@/lib/analytics/kpiEngine";
    ...
    >
      <PageSection variant="bare">
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          ...
          <KPICard title={t.kpi.uniqueUsers} value={fmtNumber(kpis.uniqueUsers, locale)} icon={Users} accent="blue" index={1} loading={isLoading} />
      ...
      <PageSection
        title={t.nav.simulations}
        description={`${fmtNumber(filtered.length, locale)} ${t.common.results}`}
        actions={
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 bg-surface-800 border border-border rounded-xl px-3 py-2">
              <Search className="w-3.5 h-3.5 text-text-muted" />
              <input
                value={search}
                onChange={(e) => { setSearch(e.target.value); setPage(1); }}
                placeholder={t.common.search}
                className="bg-transparent text-[12px] text-text-primary placeholder:text-text-muted outline-none w-44"
              />
            </div>
      ...
      >
                ...
                paginated.map((s, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3"><span className="text-xs text-text-secondary max-w-[220px] truncate block" title={s.activityName}>{s.activityName}</span></td>
                      <td className="px-4 py-3"><span className="text-xs text-text-muted whitespace-nowrap">{fmtDateTime(s.timestamp, locale)}</span></td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-14 bg-surface-700 rounded-full h-1.5 overflow-hidden">
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/dashboard/session/${s.id}`} className="text-[10px] text-brand-300 hover:text-brand-200 flex items-center gap-1">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-4 border-t border-border mt-4">
            <span className="text-[12px] text-text-muted">{t.common.page} {page} {t.common.of} {totalPages}</span>
            <div className="flex items-center gap-2">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="h-9 px-3 text-[12px] border border-border rounded-xl text-text-secondary disabled:opacity-40 hover:bg-surface-800/60 transition-colors">{t.common.previous}</button>
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="h-9 px-3 text-[12px] border border-border rounded-xl text-text-secondary disabled:opacity-40 hover:bg-surface-800/60 transition-colors">{t.common.next}</button>
            </div>
          </div>
        )}
      </PageSection>
